  ...
  public static void main( String[] arg ) {
    ...
    while ( tnum >= 0 ) {
      ...
      for ( int i = 0; i < 2; i++ ) {
        ...
        Point2D tdest = new Point2D.Double( i == 0? 700: 300, i == 0 ? 400 : 100) ;
        ...
        if ( ttimer[ i ] <= 0 ) {
          ...
          for ( int j = 0; j < plist.size(); j++ ) {
            ...
            if(i==0){ 
            if((plist.get( j ).color == Const.RED ||plist.get( j ).color == Const.BLUE  ) &&
            ...
            // to the destination.
            if ( (plist.get( j ).color == Const.GREY || plist.get( j ).color == Const.RED) &&
                 plist.get( j ).pos.distance( tdest ) > 120 &&
                 Math.abs( plist.get( j ).pos.getX() - 400 ) < 340 &&
                 Math.abs( plist.get( j ).pos.getY() - 400 ) < 340 &&
                 ( target[ i ] < 0 ||
      ...
      // Make the sled drive in an elliptical path.
